...
function LineItemRow({ item, sr, accentColor, rawMaterials, products, labels, onChange, onDelete }: LineItemRowProps) {
  ...
  return (
    <tr className="border-b border-zinc-100 print:border-zinc-200">
      ...
      {/* Qty */}
      <td className="py-2 px-2 text-right w-20 print:py-1">
        <input
          type="number"
          min="0"
          value={item.quantity === 0 ? '' : item.quantity}
          onChange={e => set({ quantity: parseFloat(e.target.value) || 0 })}
          className="w-full text-right text-sm border-b border-zinc-200 focus:outline-none focus:border-current px-0.5"
          style={{ borderColor: accentColor }}
          placeholder="0"
        />
      </td>
      ...
      {/* Unit price */}
      <td className="py-2 px-2 text-right w-28 print:py-1">
        <input
          type="number"
          min="0"
          step="0.01"
      ...
      {/* Total price */}
      <td className="py-2 px-2 text-right w-28 font-medium text-sm print:py-1">
          ...
          <table className="w-full border-collapse text-sm mb-4">
            <thead>
              <tr style={{ backgroundColor: accentColor + '18' }}>
                ...
                <th className="py-2 px-2 text-right text-xs font-semibold w-20" style={{ color: accentColor }}>Qty</th>
                <th className="py-2 px-2 text-right text-xs font-semibold w-28" style={{ color: accentColor }}>Unit price</th>
                <th className="py-2 px-2 text-right text-xs font-semibold w-28" style={{ color: accentColor }}>Total price</th>
